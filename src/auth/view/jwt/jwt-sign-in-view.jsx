...
export function JwtSignInView() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      console.error(error);
      setErrorMsg(typeof error === 'string' ? error : error?.error?.description || error?.message);
    }
  });
  ...
  const renderForm = (
    <Box gap={3} display="flex" flexDirection="column">
      ...
      <Box gap={1.5} display="flex" flexDirection="column" sx={{mt: 2 }}>
      ...
      <Box
        sx={{
          ...
          justifyContent: 'space-between', // Space items out: Checkbox on the left, Link on the right
